...
class Experiment(ShipList,
    ...
    def work_on_docked_ships(self, current_time, all_ports, all_ships):
        ...
        for ship in docked_ships:
            ...
            if port_where_ship == 0:
                ...
                ship.start_ship(current_time, all_ports, all_ships)
    ...
    def dock_ships(self, current_time, all_ports, all_ships):
        ships_docking = self.select_ships_docking(current_time)
        for ship in ships_docking:
            port = ship.read_next_destination()
            ship.stop_at_port(port, current_time, all_ports, all_ships)
        return 0

    def simulation_step(self, current_time, all_ports, all_ships):
        # this order makes docking ships stay STEP_TIME before leave to another port
        self.work_on_docked_ships(current_time, all_ports, all_ships)
        self.call_ships_to_ports(current_time)
        self.put_new_wares_to_ports(current_time)
        self.dock_ships(current_time, all_ports, all_ships)
